...
resource "terraform_data" "fetch_kubeconfig" {
  provisioner "remote-exec" {
    ...
    inline = [
      "mkdir -p ~/.ssh",
      "echo '${file("${path.module}/sshkey/my_key.pub")}' >> ~/.ssh/authorized_keys",
      "chmod 700 ~/.ssh",
...
resource "terraform_data" "download_k3s_yaml" {
  provisioner "local-exec" {
    command = "scp -i ${path.module}/sshkey/my_key -o StrictHostKeyChecking=no ubuntu@${tencentcloud_instance.web[0].public_ip}:/tmp/k3s.yaml ${path.module}/config.yaml"
  }
